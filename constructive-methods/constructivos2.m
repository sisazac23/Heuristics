...
KK = sum(aik); %Suma de los recursos de Wih para cada item i
[S,I] =sort(KK(1,1:end),'ascend'); %Criterio de selección: según peso en función objetivo
...
xlswrite("Resultados2",A,sheet,'A1'); %Escribimos número de soluciones

B = [[sum(x_i)]];
xlswrite("Resultados2",B,sheet,'A2'); %Escribimos número de elementos usados

C = [find(x_i==1)];
xlswrite("Resultados2",C,sheet,"A3"); %Escribimos índice de elementos usados

Recursos = [];
for r=1:length(bk)
   Recursos(r) = sum(aik(r,1:end).*x_i); 
end
xlswrite("Resultados2",Recursos,sheet,'A4') %Escribimos la cantidad de recursos usados por restricción

FO = [Z];
xlswrite("Resultados2",FO,sheet,"A5"); %Escribimos los valores de la función objetivo

xlswrite("Resultados2",t_fin,sheet,"A6"); %Escribimos el tiempo que tardó en resolver el problema
end
